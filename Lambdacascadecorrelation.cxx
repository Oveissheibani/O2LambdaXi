...
//
struct LambdaXiCorrelationCorrected {

  // --- Configurables ---
  Configurable<float> maxY{"maxY", 0.5, "Max |y| for Lambda and Xi"};
  
  // Configuration for correction retrieval, mimicking LambdaTableProducer
  Configurable<bool> cCorrectionFlag{"cCorrectionFlag", true, "Correction Flag"};
  Configurable<bool> cGetEffFact{"cGetEffFact", true, "Get Efficiency Factor Flag"};
  Configurable<bool> cGetPrimFrac{"cGetPrimFrac", true, "Get Primary Fraction Flag"};
  Configurable<std::string> cUrlCCDB{"cUrlCCDB", "http://ccdb-test.cern.ch:8080", "url of ccdb"};
  Configurable<std::string> cPathLambdaCCDB{"cPathLambdaCCDB", "Users/y/ypatley/lambda_corr_fact", "Path for Lambda ccdb-object"};
  Configurable<std::string> cPathXiCCDB{"cPathXiCCDB", "Users/analysis/xi_corr_fact", "Path for Xi ccdb-object"};
  ...
  TList* fLambdaCorrFactors = nullptr;
  TList* fXiCorrFactors = nullptr;

  // --- Registered Correction Histograms (To save them to output file) ---
  TH2* hSavedLambdaEff = nullptr;
  TH2* hSavedXiPurity = nullptr;

  // --- Helper: Correction Histogram Names ---
  std::vector<std::vector<std::string>> vLambdaCorrFactStrings = {
    {"hEffVsPtCentLambda", "hEffVsPtCentAntiLambda"},
  };
  std::vector<std::vector<std::string>> vLambdaPrimFracStrings = {
    {"hPrimFracVsPtCentLambda", "hPrimFracVsPtCentAntiLambda"},
  };
  
  std::vector<std::string> vXiEffStrings = {"hEffVsPtCentXiMinus", "hEffVsPtCentXiPlus"};
  std::vector<std::string> vXiPurityStrings = {"hPrimFracVsPtCentXiMinus", "hPrimFracVsPtCentXiPlus"};

  // Helper function to get correction factor from CCDB object
  float getCorrectionFactorFromCCDB(TList* ccdbObj, float cent, float pt, int particleIndex, bool isEfficiency, const std::vector<std::vector<std::string>>& names2D)
  {
    if (!cCorrectionFlag || !ccdbObj) return 1.0f;
    if (isEfficiency && !cGetEffFact) return 1.0f;
    if (!isEfficiency && !cGetPrimFrac) return 1.0f;

    // Use the 0th set of names (Pt vs Cent) from the Lambda structure convention
    const std::string& histName = names2D[0][particleIndex];

    TObject* obj = reinterpret_cast<TObject*>(ccdbObj->FindObject(histName.c_str()));
    if (!obj) return 1.0f;

    TH2* hist = dynamic_cast<TH2*>(obj);
    if (!hist) return 1.0f;

    float factor = hist->GetBinContent(hist->FindBin(cent, pt));

    // For efficiency (epsilon), the correction factor is 1/epsilon.
    if (isEfficiency && factor != 0.0f) {
      return 1.0f / factor;
    }
  ...
  // Calculates the full correction factor (Purity * 1/Efficiency) for a Xi track
  float getXiCorrection(const aod::CascDataExt::value_type& casc, float cent)
  {
    if (!cCorrectionFlag) return 1.0f;
    if (!fXiCorrFactors) return 1.0f;

    // Xi Minus is index 0, Xi Plus is index 1
    int index = (casc.sign() < 0) ? 0 : 1;
    float pt = casc.pt();

    float totalCorrection = 1.0f;

    // Efficiency (1/epsilon)
    if (cGetEffFact && fXiCorrFactors) {
        const std::string& effName = vXiEffStrings[index];
        TObject* objEff = reinterpret_cast<TObject*>(fXiCorrFactors->FindObject(effName.c_str()));
        TH2* histEff = dynamic_cast<TH2*>(objEff);
        if (histEff) {
            float eff = histEff->GetBinContent(histEff->FindBin(cent, pt));
            if (eff > 0) totalCorrection *= (1.0f / eff);
        }
    }
    
    // Purity (P)
    if (cGetPrimFrac && fXiCorrFactors) {
        const std::string& purName = vXiPurityStrings[index];
        TObject* objPur = reinterpret_cast<TObject*>(fXiCorrFactors->FindObject(purName.c_str()));
        TH2* histPur = dynamic_cast<TH2*>(objPur);
        if (histPur) {
            totalCorrection *= histPur->GetBinContent(histPur->FindBin(cent, pt));
        }
    }
  ...
  void init(InitContext const&)
  {
    // Set CCDB url and enable caching
    ccdb->setURL(cUrlCCDB.value);
    ccdb->setCaching(true);

    // --- Load CCDB Correction Factors ---
    fLambdaCorrFactors = ccdb->getForTimeStamp<TList>(cPathLambdaCCDB.value, -1);
    if (!fLambdaCorrFactors) LOGF(warning, "Lambda CCDB object not found at %s", cPathLambdaCCDB.value.c_str());

    fXiCorrFactors = ccdb->getForTimeStamp<TList>(cPathXiCCDB.value, -1);
    if (!fXiCorrFactors) LOGF(warning, "Xi CCDB object not found at %s", cPathXiCCDB.value.c_str());
    
    // --- CLONE AND REGISTER HISTOGRAMS TO BE SAVED ---

    // 1. Lambda Efficiency Histogram (Example: Lambda)
    if (fLambdaCorrFactors) {
        TObject* lambdaEffObj = fLambdaCorrFactors->FindObject(vLambdaCorrFactStrings[0][kLambda].c_str());
        if (lambdaEffObj) {
            hSavedLambdaEff = dynamic_cast<TH2*>(lambdaEffObj->Clone("hSavedLambdaEffMap"));
            if (hSavedLambdaEff) {
                // Register the cloned histogram with the task's HistogramRegistry
                histos.add("CCDB_Maps/Lambda_Efficiency_Map", hSavedLambdaEff);
            }
        }
    }

    // 2. Xi Purity Histogram (Example: Xi Minus Purity)
    if (fXiCorrFactors) {
        TObject* xiPurityObj = fXiCorrFactors->FindObject(vXiPurityStrings[0].c_str()); // Index 0 for XiMinus
        if (xiPurityObj) {
            hSavedXiPurity = dynamic_cast<TH2*>(xiPurityObj->Clone("hSavedXiPurityMap"));
            if (hSavedXiPurity) {
                 // Register the cloned histogram with the task's HistogramRegistry
                histos.add("CCDB_Maps/XiMinus_Purity_Map", hSavedXiPurity);
            }
        }
    }

    // --- Axis Definitions and Histogram Booking (Final physics histograms) ---
    const AxisSpec dphi{72, -PIHalf, 3 * PIHalf, "#Delta#varphi"};
    const AxisSpec dy{40, -2.0f, 2.0f, "#Delta y"};
    const AxisSpec pt{100, 0, 10, "p_{T} (GeV/c)"};
    const AxisSpec rap{100, -1.0, 1.0, "y"};
  ...
  template <typename L>
  void analyzeSinglesLambda(L const& tracks, float cent)
  {
    for (const auto& track : tracks) {
      if (std::abs(track.rap()) > maxY) continue;
      
      // Use the pre-calculated corrFact from LambdaTableProducer
      float w = track.corrFact();
      bool isAnti = (track.v0Type() == 1);
  ...
  template <typename T, typename F>
  void analyzeSinglesXi(T const& cascades, F const& flagsStart, float pvX, float pvY, float pvZ, float cent)
  {
    for (const auto& casc : cascades) {
      if ((flagsStart + casc.globalIndex()).isSelected() == 0) continue;
      ...
      // Calculate the correction weight for the single Xi particle
      float w = getXiCorrection(casc, cent);
      ...
      // Separate by Charge (Sign)
      if (casc.sign() < 0) {
         histos.fill(HIST("Singles/XiMinus/hPtVsMass"), casc.mXi(), casc.pt(), w);
         histos.fill(HIST("Singles/XiMinus/hRap"), xiY, w);
         
      } else {
         histos.fill(HIST("Singles/XiPlus/hPtVsMass"), casc.mXi(), casc.pt(), w);
         histos.fill(HIST("Singles/XiPlus/hRap"), xiY, w);
      }
    }
  }
  ...
  void analyzePairs(L const& lambdas, C const& cascades, F const& flagsStart, float cent)
  {
    for (const auto& lam : lambdas) {
      ...
      // The Lambda correction factor is assumed to be set by LambdaTableProducer
      float wLam = lam.corrFact();
      bool isAntiLam = (lam.v0Type() == 1);
      ...
      for (const auto& casc : cascades) {
        ...
        // Calculate Xi correction
        float wXi = getXiCorrection(casc, cent);
        float wPair = wLam * wXi;

        float dphi = RecoDecay::constrainAngle(casc.phi() - lam.phi(), -PIHalf);
        float dy   = xiY - lam.rap();
        bool isXiPlus = (casc.sign() > 0);

        if (!isAntiLam && !isXiPlus)      histos.fill(HIST("Pairs/Lam_XiM/hDeltaPhiDeltaY"), dphi, dy, wPair);
        else if (!isAntiLam && isXiPlus)  histos.fill(HIST("Pairs/Lam_XiP/hDeltaPhiDeltaY"), dphi, dy, wPair);
        else if (isAntiLam && !isXiPlus)  histos.fill(HIST("Pairs/AntiLam_XiM/hDeltaPhiDeltaY"), dphi, dy, wPair);
        else if (isAntiLam && isXiPlus)   histos.fill(HIST("Pairs/AntiLam_XiP/hDeltaPhiDeltaY"), dphi, dy, wPair);
      }
    }
  }
  ...
  void process(LambdaCollisionsExt::iterator const& lambdacoll,
               ...
               aod::CascadeFlags const& cascflags)
  {
    // Do not include the generic R2 helper function since it is not part of the class methods.
    ...
    // 1. Analyze Singles
    analyzeSinglesLambda(lambdasInThisEvent, cent);
    analyzeSinglesXi(cascadesInThisEvent, flagsStart, pvX, pvY, pvZ, cent);
    
    // 2. Analyze Pairs
    analyzePairs(lambdasInThisEvent, cascadesInThisEvent, flagsStart, cent);
  }

  // The static helper function is removed as it is not a class method.
  
  PROCESS_SWITCH(LambdaXiCorrelationCorrected, process, "Λ–Ξ correlation (CCDB Corrected)", true);
};
